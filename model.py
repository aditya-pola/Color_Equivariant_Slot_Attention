...
class CEEncoder(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        xs = x.shape
        x = x.view(xs[0],xs[3] * xs[4], xs[1] * xs[2])
...
class SlotAttentionAutoEncoder(nn.Module):
    ...
    def forward(self, image):
        ...
        x = self.encoder_cnn(image)  # CNN Backbone. [8, 16384, 32]
...
"""Slot Attention-based auto-encoder for object discovery."""
class ColorEquivariantSlotAttentionNet(nn.Module):
    def __init__(self, resolution, num_slots, num_iterations, hid_dim, planes, rotations, separable, ce_layers):
        """Builds the Slot Attention-based auto-encoder.
        Args:
        resolution: Tuple of integers specifying width and height of input image.
        num_slots: Number of slots in Slot Attention.
